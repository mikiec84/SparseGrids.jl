...
void w_cc_l_big(long D, long Q, long nG, long nc,
    double* grid, double* A, double* w,
    short* combs, short* combs_dM, int* combl, int* combu, int* cid)
{
    ...
    for (i=0;i<nG;i++)
    {
        for (q=1;q<=Q+1;q++)
        {
            ...
            for (d=0;d<D;d++)
            {
                j = round(grid[i+d*nG]*dmi+0.5);
                if (j<1)
                    j=1;
                else if (j>dmi)
                    j = dmi;
                J[q-1+d*(Q+1)] = j-1;
                B[q-1+d*(Q+1)] = fabs(1.0-fabs(grid[i +d*nG]-dxi_cc(q,j))*(mi-1));
            }
        }

        for (ii=0;ii<cid[i];ii++)
        {
